...
def filter_content(log_line):
    # Pattern for creating resources
    creating_pattern = r'\[Account (\d+)\] ([\w.-]+)\.([\w.-]+): Still creating\.\.\.'
    still_creating_pattern = r'\[Account \d+\] ([\w.-]+)\.([\w.-]+): Still creating\.\.\.'
    apply_complete_pattern = r'Apply complete! Resources: (\d+) added, (\d+) changed, (\d+) destroyed\.'
    
    # Pattern for completed creation
    completed_pattern = r'([\w.-]+)\.([\w.-]+): Creation complete'
    
    # Check for creating resources
